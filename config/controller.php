<?php 
	...
	class Resto extends koneksi{
	    ...
	    public function register($kd_user, $name, $email, $username, $password, $confirm, $level, $avatar, $redirect)
	    {
	    	global $con;
	        global $rg;
	        ...
	        if ($rows == 0) {
	            ...
	            if ($password == $confirm) {
	                ...
	                $sql      = "INSERT INTO tb_user VALUES('$kd_user','$name','$email','$username','$password','$level', '$avatar')";
	                $query    = mysqli_query($con, $sql);
	                if ($query) {
		...
		public function getCountRowsWhere($table, $where, $whereValues){
	    	global $con;
	    	$sql = "SELECT * FROM $table WHERE $where = '$whereValues'";
	    	$query = mysqli_query($con, $sql);
	    	$rows = mysqli_num_rows($query);
